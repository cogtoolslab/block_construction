...
dstruct.boot <- dpp %>%
  group_by(targetName,phase,condition) %>%
  tidyboot_mean(column=accuracy, nboot=100)
...
m4 <- lmer(data = d.Eu, euclidean_distance ~ phase_pair * distance_type + (1 + phase_pair * distance_type | gameID) + (1 + phase_pair *distance_type  | targetName))
anova(m1,m2,m3,m4)
summary(m4)
```
...
d.conseq.boot.OEu %>%
  ...
  # geom_ribbon(aes(ymin=ci_lower, ymax = ci_upper), alpha =.25, color="#1c373e", fill="#1c373e") +
  geom_point(aes(x=phase_a.f1.quartile, y=optimal_euclidean_distance, color = phase_pair), data=d.conseq.OEu, size = 3, alpha = 0.5, color="#b1c4c9") +
  geom_smooth(method='lm', formula = y ~ poly(x,1), color="#1c373e", fill="#b1c4c9") +
