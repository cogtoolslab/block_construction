...
d.Eu.boot %>%
  ggplot(aes(x=phase_pair,y=empirical_stat, color=distance_type, fill=distance_type, group = distance_type)) +
    ...
    theme(legend.position ='FALSE', legend.title=element_blank(), text = element_text(size=18), element_line(size=1), element_rect(size=2, color="#00000"))
...
ggsave('../results/plots/block_silhouette_accuracy_seq_diff.pdf', width=15, height = 10, units='cm', useDingbats = F)
```
...
ppt.var <- read.csv('../results/csv/block_silhouette_Exp2Pilot3_ppt_variance.csv')
ppt.var

# here variance is calculated separately for conditions
ppt.var.sep <- read.csv('../results/csv/block_silhouette_Exp2Pilot3_ppt_variance_separate.csv')
ppt.var.sep
```
...
ppt.var %>% 
  ...
  #scale_color_manual(values=c("#1c373e","#77798c")) +
  #scale_fill_manual(values=c("#1c373e","#77798c")) +
  theme(legend.position = 'FALSE', text = element_text(size=18), element_line(size=1), element_rect(size=2, color="#00000")) +
