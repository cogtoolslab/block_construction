...
Model distinguishing human from agent trajectories
```{r}
??m1 <- lmer(data = d.conseq, gini ~ (nlevel^2)*(iter) + (1 | targetName))
...
human_ginis <- ginis %>% 
  filter(human==TRUE) %>% 
  mutate(nlevel.squared = nlevel^2)

m1 <- lmer(data = human_ginis, gini ~ nlevel.squared + (1 | targetName))
m2 <- lmer(data = human_ginis, gini ~ nlevel.squared*phase + (1 | targetName))
anova(m1, m2)
summary(m2)
```
...
```{r}
ginis %>%
  ggplot(aes(x=nlevel, y=gini, group=phase)) +
    geom_line(aes(color=phase), width = 0, size = 1.5,
                 data = ginis %>% filter(human == 'TRUE') %>% group_by(phase)) +  
    # geom_line(width = 0, size = 1.5,
    #              data = ginis %>% filter(human == 'FALSE')) +
    ylab("structure (G)") +
    xlab("n") +
    theme_few() +
    #xlim(c(0.5,3.5)) +  
    ...
    #scale_x_discrete(labels= c(1,2,3)) + 
    theme(legend.position ='FALSE', legend.title=element_blank(), text = element_text(size=16), element_line(size=1), element_rect(size=2, color="#00000"))
#ggsave('../results/plots/.pdf', width=7, height = 10, units='cm', useDingbats = F)
```
