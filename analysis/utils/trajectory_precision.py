...
class GenericNode:
    ...
    def __init__(self, discrete_world, f1_score, target_name):
        ...
        self.x = scoring.get_precision(1*np.logical_not(target_maps[target_name]),discrete_world)
        if np.isnan(self.x):
            self.x = 1
...
class GenericBuildGraph:
    def __init__(self, target_name='no_target_name_given'):
        ...
        self.root = self.world_layers[0].get_world_node(np.zeros((18,13)).astype(int), 0, target_name=target_name) # start with one node with empty world
    ...
    def add_block_placement(self, row):
        ...
        new_node = layer.get_world_node(row.discreteWorld, row.rawF1DiscreteScore, target_name=row.targetName)
