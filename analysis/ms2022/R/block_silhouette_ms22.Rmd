...
df.full = read.csv(paste('../../results/silhouette/csv/block_silhouette_',iterationName,'_good.csv', sep=''))
...
```{r import.actions}
# df.action.metrics.full = read.csv("../../results/silhouette/csv/block_silhouette_Exp2Pilot3_all_rep_distances.csv") #cogsci
df.action.metrics.full = read.csv("../../results/silhouette/csv/df_action_distances_actual_repeated_only.csv") #ms
...
# Actions with baseline (new for ms)

Distribution of baseline values
```{r import.baselines}
df.action.baselines.raw = read.csv("../../results/silhouette/csv/df_action_distances_baseline_distributions_permuted_ppts_1000.csv")

df.action.baselines.means <- df.action.baselines.raw %>% 
  filter(phase_pair %in% c('pre to repetition 1', 'repetition 1 to repetition 2','repetition 2 to post')) %>% 
  mutate(phase_pair = ordered(phase_pair,levels=c('pre to repetition 1','repetition 1 to repetition 2','repetition 2 to post'))) %>% within(df.action.baselines.raw, phase_pair <- relevel(phase_pair, ref = 'pre to repetition 1')) %>% 
  group_by(iteration, phase_pair) %>% 
  summarise_at(vars('optimal_euclidean_distance','actual_euclidean_distance'), c(mean = mean))
  
df.action.baselines.means


```
Pivot table 
```{r}
df.action.baselines.means.pivot <- df.action.baselines.means %>% 
  pivot_longer(cols = c(actual_euclidean_distance_mean, optimal_euclidean_distance_mean), names_to='distance_type', values_to='euclidean_distance')

df.action.baselines.means.pivot
```
...
Bootstrap CIs 
```{r}
# d.baseline.boot <- df.action.baselines.means.pivot %>% 
#   group_by(phase_pair, distance_type) %>%
#   tidyboot_mean(column=euclidean_distance, nboot=1000)
# 
# d.baseline.boot
...
df.action.baselines.means.pivot %>% 
  ggplot(aes(x=phase_pair, y=euclidean_distance)) + 
  geom_line(aes(group=iteration,
              color=distance_type),
            data = df.action.baselines.means.pivot %>% 
            filter(distance_type == 'actual_euclidean_distance_mean'),
            size=0.1, 
            alpha=0.1) +
  geom_line(aes(group=iteration,
              color=distance_type),
            data = df.action.baselines.means.pivot %>% 
            filter(distance_type == 'optimal_euclidean_distance_mean'),
            size=0.1, 
            alpha=0.1) +
  geom_errorbar(aes(y=mean, color=distance_type, ymin=ci_lower, ymax = ci_upper), width = 0, size = 1.2,
               data = d.Eu.boot %>% filter(distance_type == 'actual')) +
  geom_errorbar(aes(y=mean, color=distance_type, ymin=ci_lower, ymax = ci_upper), width = 0, size = 1.2,
               data = d.Eu.boot %>% filter(distance_type == 'optimal')) +
  geom_line(aes(y=mean, color=distance_type, group=distance_type), size=1.2, data = d.Eu.boot) +
  geom_point(aes(y=mean, color=distance_type), size = 3, data = d.Eu.boot) +
  ylab("Euclidean distance") +
  xlab("pair of attempts") +
  theme_few() +
  ylim(c(0,4.5)) +
  # scale_y_continuous(breaks = seq(-0.5,2.5,0.5)) +
  scale_color_manual(values=c( "actual_euclidean_distance_mean"= "#77798c",
                               "actual"= "#77798c",
                               "optimal_euclidean_distance_mean"= "#1c373e",
                               "optimal"= "#1c373e")) +
  scale_fill_manual(values=c( "actual_euclidean_distance_mean"= "#77798c",
                               "actual"= "#77798c",
                               "optimal_euclidean_distance_mean"= "#1c373e",
                               "optimal"= "#1c373e")) +
  scale_x_discrete(labels= c("(1st, 2nd)","(2nd, 3rd)","(3rd, 4th)")) +
  theme(legend.position ='FALSE', legend.title=element_blank(), text = element_text(size=16), element_line(size=1), element_rect(size=2, color="#00000"))

ggsave('../../../analysis/results/silhouette/plots/block_silhouette_Euclidean_distances_baselines_linecloud.pdf', width=7, height = 10, units='cm', useDingbats = F)

```


Plot individual baselines as a cluster of lines to give sense of distribution
```{r plot.baselines}
df.action.baselines.means.pivot %>% 
  ggplot(aes(x=phase_pair, y=euclidean_distance, color=distance_type)) +
    # geom_line(aes(y = mean_euclidean_distance)) + 
    # geom_errorbar(aes(ymin=ci_lower, ymax = ci_upper), width = 0, size = 1.2,
                 # data = d.baseline.boot %>% filter(distance_type == 'actual_euclidean_distance_mean')) +
    # geom_errorbar(aes(ymin=ci_lower, ymax = ci_upper), width = 0, size = 1.2,
                 # data = d.baseline.boot %>% filter(distance_type == 'optimal_euclidean_distance_mean')) +
    geom_line(size=1.2) +
    geom_point(size = 3.2, fill="white", shape=21, stroke=1.2) +
    geom_errorbar(aes(ymin=ci_lower, ymax = ci_upper), width = 0, size = 1.2,
                 data = d.Eu.boot %>% filter(distance_type == 'actual')) +  
    geom_errorbar(aes(ymin=ci_lower, ymax = ci_upper), width = 0, size = 1.2,
                 data = d.Eu.boot %>% filter(distance_type == 'optimal')) +
    geom_line(size=1.2, data = d.Eu.boot) +
    geom_point(size = 3.6, data = d.Eu.boot) +
    ylab("Euclidean distance") +
    xlab("pair of attempts") +
    theme_few() +
    #xlim(c(0.5,3.5)) +  
...
# ggsave('../../../analysis/results/silhouette/plots/block_silhouette_Euclidean_distances_baselines.pdf', width=7, height = 10, units='cm', useDingbats = F)
```


Plot basline distributions
```{r plot.baselines}
d.baseline.boot %>% 
  ggplot(aes(x=phase_pair, y=mean, group=distance_type, color=distance_type)) +
    # geom_line(aes(y = mean_euclidean_distance)) + 
    geom_errorbar(aes(ymin=ci_lower, ymax = ci_upper), width = 0, size = 1.2,
                 data = d.baseline.boot %>% filter(distance_type == 'actual_euclidean_distance_mean')) +
    geom_errorbar(aes(ymin=ci_lower, ymax = ci_upper), width = 0, size = 1.2,
...
d.conseq %>% 
  ...
  theme(legend.position = 'FALSE',text = element_text(size=16), element_line(size=1), element_rect(size=2, color="#00000"))
ggsave('../../results/silhouette/plots/ms22/block_silhouette_accuracy_seq_diff.pdf', width=7, height = 10, units='cm', useDingbats = F)
...
ppt.var <- read.csv('../../results/silhouette/csv/block_silhouette_Exp2Pilot3_all_ppt_variance.csv')
ppt.var
...
ppt.var %>% 
  ...
  geom_line(size=1.2, color="#1c373e") +
  geom_point(size = 3.6, color="#1c373e") +
...
ggsave('../../results/silhouette/plots/ms22/block_silhouette_ppt_variance.pdf', width=7, height = 10, units='cm', useDingbats = F)
```
...
iterationName = 'Exp2Pilot3_all'
file_path = paste('../../results/silhouette/csv/block_silhouette_',iterationName,'_ngram_gini_final.csv', sep='')
...
```{r}
# agent_gini_boot <- mean_agent_ginis %>% group_by(nlevel) %>% tidyboot_mean(column=mean.gini, nboot=1000)
# 
# agent_gini_boot
...
```{r}
# read in human gini values from perfect trials only
iterationName = 'Exp2Pilot3_all'
file_path = paste('../../results/silhouette/csv/block_silhouette_',iterationName,'_ngram_gini_perfect_final.csv', sep='')

ginis_perf <- read_csv(file_path) %>%
  mutate(phase = factor(phase,levels=c("pre","post","random agent")))
...
```{r}
# Run same models as above but only on ginis calculated from perfect trials
mp.1 <- lm(gini~ nlevel*phase + poly(nlevel,1), 
                data = ginis_perf)
mp.2 <- lm(gini~ nlevel*phase + poly(nlevel,2), 
                data = ginis_perf)
...
```{r}
mean_perfect_human_ginis <- ginis_perf %>% 
  group_by(nlevel,phase) %>% 
  summarise_at(vars(gini),
               list(mean.gini = mean))

mean_perfect_human_ginis
```
