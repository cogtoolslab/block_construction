...
```{r import.actions}
df.action.metrics.full = read.csv("../../results/silhouette/csv/block_silhouette_Exp2Pilot3_all_rep_distances.csv")
...
```{r import.actions}
df.action.metrics.full.bl = read.csv("../../results/silhouette/csv/block_silhouette_Exp2Pilot3_all_rep_distances_w_baseline.csv")

df.action.metrics.full.bl
```

```{r}
df.Euclidean.long.bl <- df.action.metrics.full.bl %>% 
  # mutate(phase.pair.numeric = as.numeric(phase_pair)) %>% 
  select(gameID,targetName, phase_a_rawF1DiscreteScore, phase_pair,actual_euclidean_distance,optimal_euclidean_distance, baseline) %>%
  pivot_longer(c(optimal_euclidean_distance, actual_euclidean_distance), names_to='distance_type') %>% 
  rename("euclidean_distance" = "value")

df.Euclidean.long.bl$distance_type = gsub("\\_.*","",df.Euclidean.long.bl$distance_type)
```


```{r}

d.Eu.bl <- df.Euclidean.long.bl %>% 
  filter(phase_pair %in% c('pre to repetition 1', 'repetition 1 to repetition 2','repetition 2 to post')) %>% 
  mutate(phase_pair = ordered(phase_pair,levels=c('pre to repetition 1','repetition 1 to repetition 2','repetition 2 to post'))) %>% within(df.Euclidean.long.bl, phase_pair <- relevel(phase_pair, ref = 'pre to repetition 1'))

d.Eu.boot.bl<-d.Eu.bl %>%
  group_by(baseline, phase_pair, distance_type) %>%
  tidyboot_mean(column=euclidean_distance, nboot=1000)
...
d.Eu.boot.bl %>%
  ggplot(aes(x=phase_pair,y=empirical_stat, color=distance_type, fill=distance_type, group = distance_type)) +
    geom_errorbar(aes(ymin=ci_lower, ymax = ci_upper), width = 0, size = 1.5,
                 data = d.Eu.boot.bl %>% filter(distance_type == 'actual', baseline == 'no')) +  
    geom_errorbar(aes(ymin=ci_lower, ymax = ci_upper), width = 0, size = 1.5,
                 data = d.Eu.boot.bl %>% filter(distance_type == 'optimal', baseline == 'no')) +
    geom_line(size=1.5, aes(ymin=ci_lower, ymax = ci_upper), width = 0, size = 1.5,
                 data = d.Eu.boot.bl %>% filter(baseline == 'no')) +
    geom_point(size = 4, aes(ymin=ci_lower, ymax = ci_upper), width = 0, size = 1.5,
                 data = d.Eu.boot.bl %>% filter(baseline == 'no')) +
    geom_errorbar(aes(ymin=ci_lower, ymax = ci_upper), width = 0, size = 1.5, alpha=0.8,
                 data = d.Eu.boot.bl %>% filter(distance_type == 'actual', baseline == 'random_ppt')) +  
    geom_errorbar(aes(ymin=ci_lower, ymax = ci_upper), width = 0, size = 1.5, alpha=0.8,
                 data = d.Eu.boot.bl %>% filter(distance_type == 'optimal', baseline == 'random_ppt')) +
    geom_line(size=1, aes(ymin=ci_lower, ymax = ci_upper), width = 0, linetype = "dashed", alpha=0.8,
                 data = d.Eu.boot.bl %>% filter(baseline == 'random_ppt')) +
    geom_point(size = 4, aes(ymin=ci_lower, ymax = ci_upper), width = 0, linetype = "dashed", alpha=0.8,
                 data = d.Eu.boot.bl %>% filter(baseline == 'random_ppt')) +
    ylab("Euclidean distance") +
    xlab("attempt pair") +
    theme_few() +
...
```
```{r}
m1 <- lmer(data = d.Eu.bl, euclidean_distance ~ phase_pair + (1 | gameID) + (1 | targetName))
m2 <- lmer(data = d.Eu.bl, euclidean_distance ~ phase_pair + distance_type + (1 | gameID) + (1 | targetName))
m3 <- lmer(data = d.Eu.bl, euclidean_distance ~ phase_pair + distance_type + baseline + (1 | gameID) + (1 | targetName))
m4 <- lmer(data = d.Eu.bl, euclidean_distance ~ phase_pair + distance_type * baseline  + (1 | gameID) + (1 | targetName))
m5 <- lmer(data = d.Eu.bl, euclidean_distance ~ phase_pair * distance_type * baseline  + (1 | gameID) + (1 | targetName))
...
m1 <- lmer(data = d.Eu.bl, euclidean_distance ~ phase_pair + (1 | gameID) + (1 | targetName))
m2 <- lmer(data = d.Eu.bl, euclidean_distance ~ phase_pair + distance_type + (1 | gameID) + (1 | targetName))
m3 <- lmer(data = d.Eu.bl, euclidean_distance ~ phase_pair * distance_type + (1 | gameID) + (1 | targetName))
m4 <- lmer(data = d.Eu.bl, euclidean_distance ~ phase_pair * distance_type + (1 | gameID) + (1 | targetName))
anova(m1,m2,m3,m4)
summary(m4)
```
