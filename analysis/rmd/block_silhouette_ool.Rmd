...
```{r}
dpp.boot <- dpp %>% group_by(pre.f1.quartile) %>% tidyboot_mean(column=temporal_similarity, nboot=100)
dpp.boot
```
...
dpp.boot %>%
  ...
  geom_point(aes(x=pre.f1.quartile, y=temporal_similarity), data=dpp, size = 3, alpha = 0.5, color="#b1c4c9") +
...
```{r}
dpp.boot.Eu <- dpp %>% group_by(pre.f1.quartile) %>% tidyboot_mean(column=optimal_euclidean_distance, nboot=100)
dpp.boot.Eu
```
...
dpp.boot.Eu %>%
  ggplot(aes(x=pre.f1.quartile,y=empirical_stat)) +
  # geom_ribbon(aes(ymin=ci_lower, ymax = ci_upper), alpha =.25, color="#1c373e", fill="#1c373e") +
  geom_point(aes(x=pre.f1.quartile, y=optimal_euclidean_distance), data=dpp, size = 3, alpha = 0.5, color="#b1c4c9") +
  geom_smooth(method='lm', formula = y ~ poly(x,1), color="#1c373e", fill="#b1c4c9") +
  geom_point(aes(x=pre.f1.quartile, y=empirical_stat),color="#1c373e",size = 5) +
  ylab("temporal similarity between pre to post") +
  xlab("accuracy quartile (pre)") +
...
```{r}
dpp %>% 
  select(pre.f1.quartile,optimal_euclidean_distance)
```