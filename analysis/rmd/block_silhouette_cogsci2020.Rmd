...
```{r}
b.boot <- b %>% group_by(condition, repetition) %>% tidyboot_mean(column=bis, nboot=1000)
b.boot
```
...
```{r}
q.boot %>%
  ggplot(aes(x=quartile,y=empirical_stat)) +
  # geom_ribbon(aes(ymin=ci_lower, ymax = ci_upper), alpha =.25, color="#1c373e", fill="#1c373e") +
  # geom_point(aes(x=quartile, y=empirical_stat),color="#1c373e") +
  # geom_point(size = 4) +
  geom_smooth(method='lm', formula = y ~ poly(x,1)) +
  ylab("proportion actions reused (pre to post)") +
  xlab("reconstruction accuracy (pre)") +
  theme_few() +
