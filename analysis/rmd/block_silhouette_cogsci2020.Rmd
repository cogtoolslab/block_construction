...
ggsave('../results/plots/block_silhouette_reuse_quartile.pdf', width=7, height = 10, units='cm')

```

## Bootstrap resample reused proportion by quartile
```{r}
d.boot <- d %>% group_by(condition, repetition_1) %>% tidyboot_mean(column=numBlocks, nboot=100)
d.boot
```
...
d.boot %>%
  ggplot(aes(x=repetition_1,y=empirical_stat, color=condition, fill=condition)) +
    geom_ribbon(aes(ymin=ci_lower, ymax = ci_upper), alpha =.25, color=NA,
                data = d.boot %>% filter(condition == 'repeated')) + # position = dodge,
    geom_line(size=1.5)+ 
    geom_errorbar(aes(ymin=ci_lower, ymax = ci_upper), width = 0, size = 1.5,
                  data = d.boot %>% filter(condition == 'control')) +
    geom_point(size = 5) +
    ylab("number of blocks") +
    xlab("repetition") +
    theme_few() +
    scale_x_continuous(breaks = round(seq(min(d.boot$repetition_1), max(d.boot$repetition_1), by =1),1)) +
    scale_y_continuous(breaks = seq(-0.5,2.5,0.5)) +
    scale_color_manual(values=c("#77798c", "#1c373e")) +
    scale_fill_manual(values=c("#77798c", "#1c373e")) +
    theme(legend.position = c(0.75, 0.2), text = element_text(size=14), element_line(size=1), element_rect(size=2, color="#00000"))
ggsave('../results/plots/block_silhouette_numBlocks_timeseries.pdf', width=7, height = 10, units='cm')
```
