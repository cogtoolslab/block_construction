...
d.blockFell <- dpp %>% 
  group_by(phase) %>% 
  tidyboot_mean(column=blockFell, nboot=100)
...
```{r}
dpp.boot %>%
  ggplot(aes(x=phase_number,y=empirical_stat, color=condition, fill=perfectScore, linetype=perfectScore)) +
    geom_errorbar(aes(ymin=ci_lower, ymax = ci_upper), width = 0, size = 1.5,
                data = dpp.boot %>% filter(condition == 'repeated')) + # position = dodge,
    geom_line(size=1.5) +
    geom_errorbar(aes(ymin=ci_lower, ymax = ci_upper), width = 0, size = 1.5,
    ...
    scale_color_manual(values=c("#77798c", "#1c373e")) +
    scale_fill_manual(values=c("#77798c", "#1c373e")) +
    theme(text = element_text(size=18), element_line(size=1), element_rect(size=2, color="#00000"))
#ggsave('../results/plots/cogsci/block_silhouette_buildTime_timeseries.pdf', width=7, height = 10, units='cm', useDingbats = F)
```
...
# Number of blocks
```{r}
dpp.nblocks.boot <- d %>% group_by(phase_number,condition) %>% tidyboot_mean(column=numBlocks, nboot=100)
dpp.nblocks.boot %>%
