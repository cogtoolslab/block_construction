...
```{r}
## Trying a logistic regression on whether or not chunk is built by remaining participant, rather than using the count (across all participants)
m1 <- glmer(data = df.proc.chunks.perf.only, family=binomial, built_by_ppt ~ chunk_window + (1 | gameID) + (1 + phase | targetName))
m2 <- glmer(data = df.proc.chunks.perf.only, family=binomial, built_by_ppt ~ phase + chunk_window + (1 | gameID) + (1 + phase | targetName))
m3 <- glmer(data = df.proc.chunks.perf.only, family=binomial, built_by_ppt ~ phase*chunk_window + (1 | gameID) + (1 + phase | targetName))
anova(m1,m2,m3)
summary(m3)
```
