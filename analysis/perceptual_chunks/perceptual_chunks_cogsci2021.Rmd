...
```{r import.perc}
df.perceptual = read.csv('../results/perceptual_chunks/csv/df_perceptual_in_procedures.csv') %>% 
  mutate(phase=factor(phase, levels=c('pre','post')))
```
```{r}
...
```{r}
## fit model to data
m1 <- lmer(data = df.perceptual.top, proportion_with_chunk ~ phase + (1 + phase | targetName))
summary(m1)
```
